...
class AskHttpController:
    ...
    def ask(self) -> Response | tuple[Any, int]:
        ...
        try:
            if stream:
                contexts, stream_gen = self._rag.ask_stream(
                    query, enhance_strategy=strategy
                )
            ...
            return jsonify(self._rag.ask_once(query, enhance_strategy=strategy))
...
def main() -> None:
    ...
    for attempt in range(OLLAMA_502_RETRY_TIMES):
        try:
            _ctx, _gen = _rag.ask_stream("MMKV和sharedpreferences的性能对比")
            for part in _gen:
